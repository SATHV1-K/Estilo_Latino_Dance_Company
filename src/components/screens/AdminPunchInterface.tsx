...
import { Search, QrCode, User, CreditCard, Check, X, Shield, Cake, Camera } from 'lucide-react';
...
export function AdminPunchInterface() {
  ...
  const [customerStatus, setCustomerStatus] = useState<{ isBirthdayToday: boolean; hasCheckedInToday: boolean }>({ isBirthdayToday: false, hasCheckedInToday: false });
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (results.users.length > 0) {
        ...
        setMessage({ type: 'error', text: 'Customer not found. Try name, email, phone, or QR code.' });
      }
    } catch (error) {
  ...
  const handleCameraScan = async (scannedCode: string) => {
    ...
    setQrInput(scannedCode); // Show the scanned code in the input
    ...
    try {
      const results = await checkInService.searchCustomers(scannedCode);
  ...
  const handleBirthdayCheckIn = async () => {
    ...
    try {
      ...
      setMessage({
        type: 'success',
        text: `🎂 Happy Birthday! Checked in ${selectedUser.firstName} ${selectedUser.lastName} - FREE class!`
      });
      ...
      <div className="min-h-screen bg-brand-black pb-24">
        ...
        <div className="bg-brand-black border-b border-gray-700 px-6 py-4 sticky top-0 z-10">
          <div className="max-w-sm mx-auto">
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-brand-white text-xl">Admin - Punch Cards</h1>
            </div>
        ...
        <div className="px-6 py-8">
          <div className="max-w-sm mx-auto space-y-6">
            ...
            {searchMode === 'search' && (
              <Card padding="large">
                <h3 className="text-brand-black font-medium mb-4">Search Customer</h3>
                <div className="space-y-4">
                  <Input
                    label="Name, Email, Phone, or QR Code"
                    value={searchQuery}
            ...
            {searchMode === 'qr' && (
              <Card padding="large">
                <h3 className="text-brand-black font-medium mb-4">Scan Customer QR Code</h3>
                <div className="space-y-4">
                  <div className="bg-gray-900 rounded-lg p-8 border-2 border-dashed border-gray-700 text-center">
                    <QrCode size={64} className="text-brand-yellow mx-auto mb-4" />
                    <p className="text-gray-400 text-sm mb-4">
                      Scan using camera or enter QR code manually
                    </p>
                    <Button
                      variant="primary"
                  ...
                  >
                    <QrCode size={20} />
                    {loading ? 'Processing...' : 'Verify Manual Code'}
                  </Button>
                </div>
              </Card>
            )}
              ...
              <Card padding="large" className="border-brand-yellow">
                <div className="space-y-4">
                  ...
                  {customerStatus.isBirthdayToday ? (
                    ...
                    /* Regular Check-in - Show Card Info */
                    <>
                      {userCard ? (
                        <>
                          <div>
                            <div className="flex items-center gap-2 mb-3">
                              ...
                              <h4 className="text-brand-black">Active Card</h4>
                            </div>

                            <div className="bg-gray-100 rounded-lg p-4 border border-gray-300">
                              <p className="text-brand-black mb-2">{userCard.name}</p>
                              ...
                              <div className="flex justify-between text-sm mt-1">
                                ...
                                <span className="text-gray-700">{userCard.expirationDate}</span>
                              </div>
                            </div>
                          </div>
                          ...
                          {(userCard.classesRemaining <= 0 || userCard.isExpired) && (
                            <p className="text-red-400 text-sm text-center">
                              Customer needs a new card - Use Modify tab to create one
                            </p>
                          )}
                        </>
                      ) : (
                        <div className="text-center py-6">
                          ...
                          <p className="text-gray-600 text-sm">No active punch card found</p>
                          <p className="text-gray-500 text-xs mt-2">
                            Use the Modify tab to create a card for this customer
                          </p>
                        </div>
                      )}
                    </>
            ...
            <Card padding="medium" className="bg-gray-900 border-gray-700">
              <div className="flex items-start gap-2">
                <Shield size={16} className="text-brand-yellow mt-0.5 flex-shrink-0" />
                <div>
                  <h4 className="text-brand-yellow text-sm mb-1">Admin Access</h4>
                  <p className="text-gray-400 text-xs">
                    You have full access to punch cards and manage customers. Use the Modify tab to create cards for existing customers who need a new one.
                  </p>
                </div>
              </div>
            </Card>
