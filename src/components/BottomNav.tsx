...
import { Home, CreditCard, History, UserCircle, Settings, Edit, LogOut } from 'lucide-react';
...
export function BottomNav({ activeTab, onTabChange, userRole = 'customer' }: BottomNavProps) {
  ...
  // Staff: Home, History, Profile (3 icons)
  // Admin: Home, Modify, History, Admin, Profile (5 icons)
  ...
  const getTabsForRole = () => {
    if (userRole === 'customer') {
      ...
      return [
        { id: 'home' as NavTab, icon: Home, label: 'Home' },
        { id: 'history' as NavTab, icon: History, label: 'History' },
        { id: 'profile' as NavTab, icon: LogOut, label: 'Logout' }
      ];
    } else {
      // admin
      return [
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center justify-center gap-1 px-4 py-2 rounded-lg transition-colors duration-200 min-w-16 touch-target ${isActive ? 'text-brand-yellow' : 'text-gray-400 hover:text-brand-white'
                  }`}
              >
